...
public class ProdutoDAO {
    ...
    public List<Produto> obterTodos() {
        ...
        Cursor cursor = banco.query("produtos", new String[]{"produtoId", "produtoNome", "produtoQuantidade", "produtoPreco"}, null, null, null, null, null);
        while (cursor.moveToNext()) {
            Produto p = new Produto();
            p.setProdutoId(cursor.getInt(0));
            p.setProdutoNome(cursor.getString(1));
            p.setProdutoQuantidade(cursor.getFloat(2));
            p.setProdutoPreco(cursor.getFloat(3));
            produtos.add(p);
        }
        return produtos;
    }
